...
def get_element_attributes(xml_file, elements: set) -> dict:
    context = etree.iterparse(xml_file, dtd_validation=True, no_network=False, events=('start', 'end'), attribute_defaults=True,
                              load_dtd=True)
...
def parse_xml(xml_file, elements: set, output_files: Dict[str, csv.DictWriter], relation_attributes: set,
    ...
    context = etree.iterparse(xml_file, dtd_validation=True, no_network=False, events=('start', 'end'))
    # turn it into an iterator
    context = iter(context)
    # get the root element
    event, root = next(context)
